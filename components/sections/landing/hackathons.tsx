...
// Full-screen image viewer with improved mobile support
function FullScreenImageViewer({
  ...
  // Enhanced backdrop click/touch handling for mobile
  const handleBackdropInteraction = useCallback(
    (event: React.MouseEvent<HTMLDivElement> | React.TouchEvent<HTMLDivElement>) => {
      // Ensure we're clicking/touching the backdrop itself, not a child element
  ...
  // Enhanced close button handling for mobile
    ...
    >
      ...
      >
        <span className="text-white text-sm font-medium">
          ...
          <span className="hidden sm:inline">Aperte ESC ou clica fora para fechar</span>
          <span className="sm:hidden">Clique no X ou fora para fechar</span>
        </span>
      </div>
    </div>
  )
...
export function HackathonsSection() {
  ...
  // Enhanced image click handler with mobile support
                          ...
                          <div className="flex justify-center gap-2 mt-4">
                            {selectedHackathon.photos.map((_, index) => (
                              <button
                                ...
                                onClick={fullScreenImage ? undefined : () => setCurrentPhotoIndex(index)}
                                onTouchEnd={fullScreenImage ? undefined : () => setCurrentPhotoIndex(index)}
                                disabled={!!fullScreenImage}
